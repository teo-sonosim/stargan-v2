...
class Solver(nn.Module):
    def __init__(self, args):
        ...
        if args.mode == "train":
            ...
            self.ckptios = [
                CheckpointIO(
                    ospj(args.checkpoint_dir, "{:06d}_nets.ckpt"), **self.nets
                ),
                CheckpointIO(
                    ospj(args.checkpoint_dir, "{:06d}_nets_ema.ckpt"), **self.nets_ema
                ),
                CheckpointIO(
...
def compute_d_loss(
    ...
    return (
        loss,
        Munch(real=loss_real.item(), fake=loss_fake.item(), reg=loss_reg.item()),
    )
